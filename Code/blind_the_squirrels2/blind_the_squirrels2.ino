...
void setup() {
    ...
    {
        Serial.println("Initializing the load cell scale");

        scale.begin(LOADCELL_DOUT_PIN, LOADCELL_SCK_PIN);
        scale.set_scale(1140.f);
        scale.tare();  // reset the scale to 0
    }
    reset_timer = millis();

    // initialize digital pin GPIO4 as an output, for the PCB LED
...
void loop() {
    ...
    if (camera_sign && sd_sign) {
        ...
        // Reset conditions every 30s
        if (millis() - reset_timer > 30000) {
            start_time = 0;
